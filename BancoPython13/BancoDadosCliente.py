...
class BancoDadosCliente():
	...
	def InserirCliente(self, Cliente): 
		...
		if(not self.VerificarUsuarioNaBaseDados(Cliente.CPF)): #Usuario nao esta na base de dados
			...
			self.cursor.execute("INSERT INTO Clientes VALUES (?,?,?,?,?,?,?)",(Cliente.CPF, Cliente.nome, Cliente.senha, Cliente.dataAbertura, Cliente.saldo, Cliente.extrato, Cliente.movimentacoes))
			self.conexao.commit()
			return True
		else: 
