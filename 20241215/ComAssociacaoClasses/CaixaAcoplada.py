...
class CaixaAcoplada(object): 
	def __init__(self):
		...
		self.Controle = CA.ControleAcionamento()
		self.ValvulaEntrada = VA.ValvulaEntradaAgua()
		self.ValvulaSaida = VS.ValvulaSaidaAgua()
	...
	def AcionarDescarga(self):
		...
		if tipo == '2':
			print("\t\t\t Fedeu?")
			Resposta = input("\t\t\t Digite 1 se sim e 2 se não. ")
			if Resposta == '1':
				self.AcionamentoPerfume += 1 #Verificação da troca
				self.Controle.set_acionamento_perfume()
			Troca = self.Controle.get_informacao_troca_perfume()
			if (Troca):
				print("Precisa trocar o refil do perfume.")
			else:
				print("Há ainda perfume no refil.")
